...
schedule length is not where the gap comes from. That figure describes an
association among the characteristics the survey records; it does not estimate
what a given worker would give up by choosing music.
...
These comparisons support one narrow conclusion: nothing in them shows Austin
paying musicians better than the other large Texas metros.
...
musicians never enter the payroll survey at all (see Section~\ref{sec:payroll}).
...
\NVenuePanelShareTwoZeroTwoFive\ in 2025.

Once each venue's own trend and seasonality are absorbed, monthly receipts at
live-music rooms are statistically indistinguishable from receipts at other
bars after March 2020: the estimated average difference is \NVenueDidPct, with
a 95 percent interval running from about 14 percent below to about 17 percent
above the rest of the market (Section~\ref{sec:venues}). The aggregate decline
therefore comes from which rooms exist, not from weaker per-room trade among
the survivors: \NVenueExitsTwoZeroTwoZeroTwoZeroTwoSixCore\ exits from the tracked
...
sharpest divider (Section~\ref{sec:census}). Most respondents who did not
answer yes to the Austin question chose ``unsure'' rather than ``no.''

\subsection*{Austin and Texas have built real music programs; the record shows how small and uneven they are}

The left column lists what the public record shows has been built; the right
column lists what the same record shows about size and reach.
...
& The fund receives \NCityLmfShareOfHotFyTwoFive\ of city hotel-tax revenue.
Historic preservation draws \NCityHpToLmfRatioFyTwoFive\ as much from the same
tax, because it receives hotel-tax money beyond the equal share of the 2019
increase the ordinance gave each fund (see Section~\ref{sec:city}). \\
\rowrule
Texas is the only state compared here that subsidises the operation of music
venues, and in the one rebate round with a published total it awarded the full
amount the state dedicates to the program each year.
& Through the same state office, Texas dedicated moving-image money and music
money for the 2026--27 biennium at a ratio of \NTxFilmMusicRatio\ (see
...
picture theatre operators received slightly more than live venues did (see Section~\ref{sec:state}).
& Neither the city's 2017 pilot extending closing hours at five Red River
venues nor its 2024 Live Music Fund venue grants left a detectable mark on
venue receipts (see Section~\ref{sec:venues}). \\
\rowrule
