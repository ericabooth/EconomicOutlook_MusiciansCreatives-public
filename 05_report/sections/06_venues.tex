...
restaurants and breweries that host occasional acts), settings that typically
pay performers less, and less regularly, than dedicated rooms do.
...
common shocks out by holding each venue and each calendar month constant,
then asks what difference remains.
...
points (roughly 42 percent) below the comparison group in 2020, returned to
within statistical
...
staying flat, so a counterfactual that continued that drift would end above
...
the wider licensed-beverage market keeps adding permit locations. Measured on
receipts alone, rooms that stayed open kept pace with comparable non-music
bars after 2019; the gap between the panel and the
...
\emph{Do512} and \emph{Texas Monthly}, compiled for this study and included
with its evidence files. Reasons are
...
total tickets sold; the evaluation memo behind those figures has not been
located, and no independent check of them has been published. The Comptroller receipts
...
suburban ones in many ways at once, so this is a description rather than a
causal estimate. Note also that any market-wide arena effect is absorbed by the
