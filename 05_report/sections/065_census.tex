...
release contains 33 fewer rows, and its documentation gives no reason. Every
share in this section uses the \NCensusNRespondents-row microdata file, so a
value here can differ by a few tenths of a point from a published percentage
even where the two agree.} That fielding is one edition of a national cohort:
Sound Music Cities asks the same core questions city by city, which lets an
Austin answer be compared, question for question, with answers from Sacramento
or Nashville (Section~\ref{sec:measurement} makes that comparison for the
income items). The 2022 instrument dropped the dollar-denominated income
...
music census has not asked what a musician earns for twelve years running.}
...
answers move together, and what one person's total spending across the eight
business categories comes to.
...
microdata file.} Most respondents who did not answer yes to the Austin
question expressed uncertainty rather than a plan to leave: among the
\NCensusRetentionDenom\ respondents,
...
of probabilities and is further from one than the probability ratio is
...
\subsection{Working in music costs money; the typical respondent spends about half the widely cited figure}
...
2025 dollars, so the correctly constructed mean is well below the widely
cited figure and the median is far below it.\footnote{The widely cited
...
accounting and legal, and web and social media; the item is asked of music
creatives only and does not appear on the venue-operator branch.} Because spending is
heavily right-skewed, the median describes a typical respondent better than
any mean: \NCensusSpendTotalMedian\
...
bound because the release cannot distinguish a true zero from a blank). That
rebuild is possible only because the census program released its
respondent-level records as open data; a summary-only release could not have
been checked at all.
